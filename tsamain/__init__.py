...
def create_app(test_config=None):
    ...
    def index():
        ...
        db = get_db()
        info = db.execute("SELECT e.eventtitle, e.eventdate, e.eventlevel, e.eventprice, d.eventdesc, d.eventhero, e.eventid FROM events e LEFT JOIN edetails d ON e.eventid = d.deventid WHERE eventdate > DATE('now', '-2 hours') ORDER BY eventdate ASC LIMIT ?", (10,)).fetchall()
        featured = db.execute(
