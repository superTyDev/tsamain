...
def authmanage():
    if request.method == 'POST':
        if 'deleteAccount' in request.form:
            ...
            error = None
            try:
                db.execute('DELETE FROM user WHERE userid = ?',
                           (request.form['deleteAccount'],))
                db.execute('DELETE FROM udetails WHERE duserid = ?',
                           (request.form['deleteAccount'],))
                db.commit()
            except db.IntegrityError:
                flash(
                    f"Error deleting account {request.form['deleteAccount']}. Please contact us.")
                return redirect(url_for("auth.login"))
            else:
                flash(f"User account {request.form['deleteAccount']} deleted.")
                return redirect("/")
